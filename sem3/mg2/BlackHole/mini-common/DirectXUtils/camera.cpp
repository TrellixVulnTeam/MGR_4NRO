...
DirectX::XMFLOAT4 OrbitCamera::getPointPosition(XMFLOAT3 point, const ViewFrustrum& frustrum) const
{
	if (m_distance == 0.0f)
		return m_target;
	XMMATRIX viewMtx = getViewMatrix();
	XMVECTOR det;
	viewMtx = XMMatrixInverse(&det, viewMtx* frustrum.getProjectionMatrix());
	//auto alt = XMMatrixTranslation(0.0f, 0.0f, -m_distance) * XMMatrixRotationY(m_angleY) * XMMatrixRotationX(-m_angleX);
	XMFLOAT3 res(point);
	auto transl = XMVector3TransformCoord(XMLoadFloat3(&res), viewMtx);
	XMStoreFloat3(&res, transl);
